...
<html>
<head>
  <title>peranKo | Hasil Perhitungan Suara</title>
  ...
  <br>
    ...
    <div class="form-group" align="center">
      ...
      <!-- <div style="background-color: red; width: 10px; height: 10px; border-radius: 2px;"></div> -->
                    ...
                    <select  align="left" name="wilayah" class="form-control select2 right  col-md-2"  id="exampleFormControlSelect2">
                      ...
                      <?php foreach ($tahun as $tahun): ?>
                         <option value="<?=$tahun->IDTAHUN?>"> <?=$tahun->NMTAHUN?></option>
                        <?php endforeach; ?>
                    </select>
                      <select  align="left" name="konflik" class="form-control select2  col-md-2"  id="exampleFormControlSelect2">
                       <option selected> -- Pilih Partai --</option>
                        <option value="all"> Semua Partai </option>
                       <?php foreach ($parpol as $pol): ?>
                         <option value="<?=$pol->IDPARPOL?>"> <?=$pol->NMPARPOL?></option>
                        <?php endforeach; ?>
                    </select>
                  </div>
    </div>
    ...
    foreach ($kab as $row ): 
      $idkab = $row->IDWIL;
        ...
        $jml_konflik = $this->db->query("SELECT COUNT(IDTRXANGKET) AS JMLKONFLIK FROM tr_angket 
               ...
               // $color = '<div style="background-color: '. $row->NMWIL.'; width: 40px; height: 40px; border-radius: 2px;">ASDA</div>' 
            $max = $this->db->query("
                   SELECT DISTINCT(a.IDWILAYAH),MAX(a.JUMLAH) AS TERTINGGI,  a.WILAYAH, a.PARTAI, a.IDPARTAI, a.LOGOP FROM (
                    SELECT SUM(TOTALSUARA) AS JUMLAH,m_kabkota.`NMWIL` AS WILAYAH ,m_kabkota.`IDWIL` AS IDWILAYAH, m_parpol.`NMPARPOL` AS PARTAI, m_parpol.`IDPARPOL` 
                    AS IDPARTAI, m_parpol.`LOGOPARPOL` AS LOGOP  FROM tb_pemilu 
                    ...
                    WHERE  tb_pemilu.`IDTAHUN` = '3' AND m_kabkota.`IDWIL` = '$idkab' 
                    GROUP BY tb_pemilu.`IDPARTAI`, m_kabkota.`IDWIL` ORDER BY TOTALSUARA DESC )a ")->row();  
                                              ?>
...
var greenIcon<?=$row->IDWIL; ?> = L.icon({
    iconUrl: '<?php echo base_url()?>assets/gambar/<?=$max->LOGOP?>',    
...
function popUp<?=$row->IDWIL; ?>(f,l){
    ...
    if (f.properties){
      ...
      out.push("<b style='width:45%;float:left'>Jumlah Konflik</b> <div style='float:left'><b>: <?php echo($jml_konflik->JMLKONFLIK)?></b></div><br />");
       out.push("<b style='width:45%;float:left'>Partai Dominan</b> <div style='float:left'><b>: <?php echo($max->PARTAI)?> ( <?php echo number_format($max->TERTINGGI)?> )</b></div><br />");
        l.bindPopup(out.join("<br />"));
    }
}
<?php
