        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <label for="message-text " class="form-control-label">Wilayah Kab/Kota</label><br>
                        <select class="form-control form-control-md col-md-12" id="in-kab">    
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control " id="in-thn"> 
                    ...
                    <div class="form-group">
                       ...
                       <input type="text" class="form-control " id="suara"  name="suara" >
                    </div>                                                                                                              
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form>
                    ...
                    <div class="form-group">
                        <label for="message-text" class="form-control-label">Partai Politik</label>
                        <select class="form-control form-control-sm" id="ed-pol">
                         <option value="" id="sl-pol" selected></option>   
                        <option value=""> -- Pilih Partai Politik --</option>                                                         
                        <?php foreach ($parpol as $row ): ?>  
                                <option value="<?=$row->IDPARPOL?>"><?=$row->NMPARPOL?></option>                               
                                 <?php endforeach ?>
                    ...
                    <div class="form-group">
                        <label for="message-text" class="form-control-label">Tahun</label>
                        <select class="form-control form-control-sm" id="ed-thn"> 
                             <option value="" id="sl-thn" selected></option>  
                        <option value=""> -- Pilih Tahun --</option>                                                         
                        <?php foreach ($tahun as $row ): ?>  
                                <option value="<?=$row->IDTAHUN?>"><?=$row->NMTAHUN?></option>                               
                                 <?php endforeach ?>
                    ...
                    <div class="form-group">
                        <label for="recipient-name"  class="form-control-label">Total Suara</label>
                       <input type="text" class="form-control form-control-sm" id="esuara"  name="suara" >
                    </div> 
                      <script type="text/javascript">        
                          var epagu = document.getElementById('esuara');
                          epagu.addEventListener('keyup', function(e){  
                       
                          epagu.value = formatRupiah(epagu.value, '');
                        });      
            ...
            <div class="modal-footer">
                <button type="button" id="val-ed" class="btn btn-secondary" data-dismiss="modal">Batal</button>
...
<div class="modal fade" id="mdelete" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-md" role="document">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-dismiss="modal">Batal</button>
                <button type="submit" id="deleted" class="btn btn-primary">Simpan</button>
            </div>
        </div>
    </div>
</div>
                ...
                <div class="col-md-4 kt-margin-b-20-tablet-and-mobile">
                 ...
                 <div class="col-md-4 kt-margin-b-20-tablet-and-mobile">
                    <div class="kt-form__group kt-form__group--inline">
                        ...
                        <div class="kt-form__control">
                         <select class="form-control" id="parpol">
                            <option value="">All</option>
                            <?php foreach ($parpol as $row ): ?>  
                                <option value="<?=$row->NMPARPOL?>"><?=$row->NMPARPOL?></option>                               
                                 <?php endforeach ?>
                            </select>
                        </div>                      
                    </div>
                    ...
                    <div class="kt-form__group kt-form__group--inline">
                        ...
                        <div class="kt-form__control">
                         <select class="form-control" id="tahun">
                            <option value="">All</option>                            
                                 <?php foreach ($tahun as $row ): ?>  
                                <option value="<?=$row->NMTAHUN?>"><?=$row->NMTAHUN?></option>                               
                                 <?php endforeach ?>
                            </select>
                        </div>                      
                    </div>
    ...
    $(document).ready(function() {     
      ...
      load();     
       ...
       table = $('#kt_table').DataTable({
            ...
            "order": [],
            "ajax": {
                "url": "<?php echo site_url('pemilu/view')?>",
                "type": "POST",
                "data": function ( data ) {
                    data.wilayah = $('#wilayah').val();    
                    data.parpol = $('#parpol').val();    
   ...
   $('#save').click(function(){
    ...
    function input(){       
      ...
      var suara = $('#suara').val();  
         ...
         $.ajax({
          type:'POST',
           url: "<?php echo site_url('pemilu/input')?>",
           data:'kec='+kec+'&pol='+pol+'&tahun='+thn+'&suara='+suara,
    ...
    $('body').on('click','.edit_btn',function(){ 
             ...
             var idthn = $(this).data("idthn");
             var pol = $(this).data("pol");
             var idpol = $(this).data("idpol");
             var suara = $(this).data("suara");

             var bid = document.getElementById('ed-thn');
            bid.selectedIndex = 0; 
             var kb = document.getElementById('ed-pol');
            kb.selectedIndex = 0; 
      ...
      $('#btn-edit').click(function(){ //button filter event click    
           ...
           var thn = $('#ed-thn').val();
           var pol = $('#ed-pol').val();
           var suara = $('#esuara').val();          
                    
     
               $.ajax({
              type:'POST',
               url: "<?php echo site_url('pemilu/edit')?>",
              data:'kec='+kec+'&thn='+thn+'&pol='+pol+'&suara='+suara+'&id='+ids,
              success: function(response){                   
                $('#kedit').modal('hide');          
                 $('#kt_subheader').html(response); 
                table.ajax.reload();               
     ...
     $('#deleted').click(function(){ //button filter event click             
         var iddel =  $('#id_delete').val();           
           $.ajax({
          type:'POST',
           url: "<?php echo site_url('pemilua/delete')?>",
           data:'id='+iddel,
          success: function(response){           
            $('#mdelete').modal('hide');          
             $('#kt_subheader').html(response); 
